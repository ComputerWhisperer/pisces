/*	makeframe.c		Version 1.12		*/
/*	Last Modification:	3/31/90 09:36:04		*/
/*----------------------------------------------------------------------
**  Copyright 1989 by
...
static char SccsID[] = "@(#)makeframe.c	1.12\t3/31/90";

/*
     input:
	data = array of data for entire array 
	nplot= number of frames in conman output file tfile (tfile is default)
	jstart= number of indexing to start from for output ( number to start
			concatinating to variable name for output frame name)
...
/* Set min and max equal to gmin and gmax for consistency in multiple files */
	...
	if (n == 0 && k == 0 && l == 0)
		sprintf(filename,"%s%d%d%d(%d*%d)",name,n,k,l,NX,NY);
	else
		sprintf(filename,"%s%d%d%d",name,n,k,l);
