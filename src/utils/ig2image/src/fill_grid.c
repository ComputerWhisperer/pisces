/*
 *	fill_grid.c		Version 1.9		
 *	Last Modification:	4/10/90 13:10:11	
 *
...
/* fix problem where we are at interface boundary */
	...
	for ( j = min_xindex; j <= max_xindex; j++ )  {
		...
		xcord = xdata[j] * 1.0e-4;
		ycord = ydata[k] * 1.0e-4;
	
		if (pt_in_tri(xcord,ycord,i))  {
			...
			value = NITRIDE_VALUE;
		    	...
		    	value = interpolate(xcord, ycord, i, val);
		    pt_num = vertex_tri(xcord, ycord, i);
