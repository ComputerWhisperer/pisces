...
#endif
{
	...
	if ( dop->type != NONE )  {
            printf("\tYou can enter doping information in 3 ways: \n");
	    ...
	    if (! ( (doping_type=='1') || (doping_type=='2') || (doping_type=='3') ) )
	        doping_type = '2';
            printf("\t\t please pick 1, 2 or 3 [%c] : ", doping_type );
            fflush(stdout);
            gets(buf);
		...
		buf[i] = doping_type;
		    ...
		    if ( (dop->dop_tag).anal_dop.peak_dop != 0.0 )  {
    	                read_real("\tPeak Doping Location", 
			    &((dop->dop_tag).anal_dop.peak_loc) );
	                read_real("\tJunction Depth", &temp );
		        (dop->dop_tag).anal_dop.std_dev =
		            ( temp - (dop->dop_tag).anal_dop.peak_loc)/
