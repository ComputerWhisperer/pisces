...

{
    ...
    double MINSHORTSPAC = minimum * MINSHORTGRID;
    double zero = 0.0;
 ...
 */
    ...
    if ( lat_sd_junc >= lat_info.poly_width/2.0 )  {
	fprintf( stderr, "\tSource/Drain will be shorted. Please fix doping\n" );
	exit( ERROR );
    }
	...
	case NONE	:
			...
			sd_width12 = 0.0; /* use a small number */
		        break;
    ...
    grid_spac = dmin( grid_spac, MINSHORTSPAC ); /* pick a lower bound in the lateral
					 *  direction
					 */
